...
stockA_df.set_index(["Date"], inplace=True)

stockA_df.index = pd.to_datetime(stockA_df.index)

stockA_df = stockA_df.iloc[stockA_df.reset_index().groupby(stockA_df.index.to_period('M'))['Date'].idxmax()]

#从尾部去掉 2 行
stockA_df = stockA_df.drop(stockA_df.tail(2).index)

code_list = []

for a in range(0,portefeuille_df.shape[0]) :
       code_list.append(portefeuille_df.iloc[a,0])

stockA_df = stockA_df.loc[:,code_list]
...
def gerber_cov_stat(rets: np.array, threshold: float=0.9) -> tuple:
    """
...
class portfolio_optimizer:
    def __init__(self, min_weight: float = 0., max_weight: float = 1.0,
        ...
        """
        # check arguments
        assert cov_function in ['HC', 'GS'], "The covariance function must be one from HC, SM, GS"
    ...
    def optimize(self, obj_function: str,
        ...
        if self.cov_function == "HC":
            ...
            self.covariance, _ = gerber_cov_stat(self.returns_df.values, threshold=self.gs_threshold)
            self.covariance_neg, _ = gerber_cov_stat(self.negative_returns_df.values, threshold=self.gs_threshold)
...
#计算每月收益率
month_pct_change = stockA_df.pct_change()

#用0填补缺失值NA
month_pct_change.fillna(0, inplace=True)

#查看每月收益率的前几行
print(month_pct_change)
rets_df = pd.DataFrame(month_pct_change)
rets = rets_df.values

#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# test objective function list
obj_function_list = ["meanVariance"]
cov_function_list = ["HC","GS"]

# "meanVariance" : 
# maximisation de la rentabilité sous contrainte d'un niveau de risque donné

print("\n")

for cov_fun in cov_function_list:
        print("MVO based on %s covariance function ..." % cov_fun)
        port_opt = portfolio_optimizer(min_weight=0, max_weight=1, cov_function=cov_fun, freq="monthly")  ### min_weight = 0
        ...
        for obj_fun_str in obj_function_list:
            ...
            print("\n")
            weights = pd.DataFrame(np.around(weights, decimals=2))
            weights.rename(columns={0:'weight'}, inplace = True)
            weights['code'] = code_list
            weights.set_index(["code"], inplace=True)
            weights['weight'] = weights['weight'].apply(lambda x: format(x, '.2%'))
            print(weights)
            print("\n")

for cov_fun in cov_function_list:
        print("MVO based on %s covariance function ..." % cov_fun)
        port_opt = portfolio_optimizer(min_weight=0.01, max_weight=1, cov_function=cov_fun, freq="monthly")  ### min_weight = 0.01
        port_opt.set_returns(returns_df=rets_df)
        # run MVO under various optimization goals
